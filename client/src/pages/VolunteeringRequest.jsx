...
const UserRequests = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        const response = await axios.get(`https://platesforall.onrender.com/user/findRequests/${user.userId}`, {
          headers: {
  ...
  const onAccept = async (reqId) => {
    try {
      const response = await axios.post(`https://platesforall.onrender.com/donation/accept/${user.userId}/${reqId}`, {}, {
        headers: {
          'Content-Type': 'application/json',
          // Include other headers as needed
        },
